    ...
    clientOptions: {
      ...
      properties: {
        tone: {
          type: 'range',
          label: 'Tone',
          hint: '1. Creative / 2. Balanced / 3. Precise',
          min: 1,
          max: 3,
          step: 1,
        },
    ...
    clientOptions: {
      ...
      properties: {
        tone: {
          type: 'range',
          label: 'Tone',
          hint: '1.Professional / 2.Casual / 3.Enthusiastic / 4.Informational / 5.Funny',
          min: 1,
          max: 5,
          step: 1,
        },
        format: {
          type: 'range',
          label: 'Format',
          hint: '1.Paragraph / 2.Email / 3.Blog / 4.Ideas',
          min: 1,
          max: 4,
          step: 1,
        },
...
const { setPreset, getPreset, deletePreset, setActivePresetName } = presetStore;
...
const generateForm = (options, parentKey, levels = 0) =>
  Object.entries(options).map(([key]) => {
    ...
    if (option.type === 'nested') {
      return h(
        'div',
        { class: 'flex flex-col gap-2', style: `margin-left: ${levels}em;` },
        [
    ...
    let classList =
      'w-full placeholder-white/40 text-slate-300 text-sm rounded py-2 focus:outline-none';
    switch (option.type) {
      case 'range':
        ...
        inputElement = h(
          Switch,
          {
            ...
            class: `relative inline-flex h-6 w-11 items-center rounded-full transition ${
              inputValue ? 'bg-white/50' : 'bg-white/10'
            }`,
          },
          () => [
            ...
            h('span', {
              class: `inline-block h-4 w-4 transform rounded-full bg-white transition ${
                inputValue ? 'translate-x-6' : 'translate-x-1'
              }`,
            }),
          ]
        );
    ...
    return h(
      ...
      [
        h(
          'label',
          { class: 'text-white/80' },
          option.type === 'range' ? `${option.label}` : option.label
        ),
        h(
          'span',
  ...
  <Dialog :open="isOpen" @close="setIsOpen(false)" class="relative z-50">
    ...
    <div class="fixed inset-0 overflow-y-auto">
      ...
      <div class="flex min-h-full items-center justify-center p-4">
        ...
        <DialogPanel
          class="w-full max-w-3xl rounded text-slate-300 bg-white/10 backdrop-blur-lg p-6 shadow-lg"
        >
          <button
          ...
          <DialogTitle
            class="font-black text-slate-100 text-2xl flex items-center"
          >
            <GPTIcon
              v-if="client === 'chatgpt'"
            ...
            <div class="flex flex-col sm:flex-row justify-end mt-4 gap-2">
              ...
              <Transition name="fade">
                <button
                  ...
                  type="button"
                  class="flex items-center justify-center px-2 py-2 rounded bg-red-500/50 text-white/70 hover:text-white/90 hover:bg-red-500/60 transition duration-300"
              ...
              <!-- Save as Name input -->
              <div
                class="relative flex flex-col sm:flex-row items-stretch shadow-inner bg-white/5 rounded"
              >
                <label
                  class="text-white/60 text-xs h-full flex items-center px-3 py-2 border-white/5 border-b sm:border-r sm:border-b-0"
                >
                  Preset Name
                </label>
                <input
                ...
                />
                <button
                  class="flex items-center justify-center px-3 py-2 group bg-white/5 sm:bg-transparent"
                  @click="resetSaveAsName"
                >
                  <Icon
                    name="bx:bx-reset"
                    class="text-white/70 group-hover:text-white/90 transition"
                  />
                </button>
              </div>
              <button
                type="button"
                class="flex items-center justify-center gap-1 py-2 text-slate-300 rounded bg-white/10 transition duration-300 ease-in-out hover:bg-white/20"
                @click="save"
              >
                <Icon
                  name="bx:bx-save"
                  class="relative text-lg top-[1px] ml-4"
                />
